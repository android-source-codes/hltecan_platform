	...
	private_kernel_netlink_plugin_t *this, plugin_feature_t *features[])
{
	...
	DBG1(DBG_KNL, "USE_KERNEL_NETLINK_IPSEC");
#else
	static plugin_feature_t f[] = {
        /* Since we are using user space IPsec, we don't want to register
        IPsec SAs or policies to kernel IPsec.*/
        PLUGIN_CALLBACK(kernel_net_register, kernel_netlink_net_create),
            PLUGIN_PROVIDE(CUSTOM, "kernel-net"),
	};
	DBG1(DBG_KNL, "NO_USE_KERNEL_NETLINK_IPSEC");
#endif
